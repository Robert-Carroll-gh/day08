...
fn all_cards() -> String {String::from("J23456789TQKA")}
...
fn part1(input: &str) -> usize {
    ...
    for direction in directions.iter().cycle() {
        key = get_next_node(direction, key, &node_map);
        steps += 1;
        if key == "ZZZ" {break};
    }
...
fn get_next_node(direction: &Direction, key: String, node_map: &HashMap::<String, (String, String)>) -> String {
    match direction {
        Direction::Left => node_map.get(&key).unwrap().0.clone(),
        Direction::Right => node_map.get(&key).unwrap().1.clone()
    }
}

fn parse_input(input: &str) -> (Vec<Direction>, HashMap::<String, (String, String)> ) {
    let (directions, nodes) = input.split_once("\n\n").unwrap();
    let directions = directions.chars().map(|character| {
        match character {
            'L' => Direction::Left,
            'R' => Direction::Right,
            _ => panic!()
    ...
    let nodes: Vec<(&str, &str, &str)> = nodes.lines().map(|line| {
        let mut ids = line.split(&[' ','(',')','=',',']).filter(|text| !text.is_empty());
        (ids.next().unwrap(),ids.next().unwrap(),ids.next().unwrap())
    }).collect();
    
    let mut nodes_map = HashMap::<String, (String, String)>::new();
    for (id, left, right) in nodes {
        nodes_map.insert(id.to_string(), (left.to_string(), right.to_string()));
    }
